        ...
        try:
            with mlflow.start_run(experiment_id=self.experiment_id) as run:
                ...
                dataset_record = TrainingDataset(
                    id=uuid.uuid4(),
                    name=f"fraud_dataset_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}",
                    version=f"{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}",
                ...
                model_record = MLModel(
                    id=uuid.uuid4(),
                    name=f"gradient_boosting_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}",
                    version=version_str,
