...
class ModelEvaluator:
    ...
    async def _calculate_recall(self, predictions, actuals):
        true_positives = sum(
            1 for p, a in zip(predictions, actuals, strict=False) if p >= 0.5 and a == 1
        )
    ...
    async def _generate_confusion_matrix(self, predictions, actuals):
        return {
            ...
            "false_positives": sum(
                1 for p, a in zip(predictions, actuals, strict=False) if p >= 0.5 and a == 0
            ),
            "true_negatives": sum(
                1 for p, a in zip(predictions, actuals, strict=False) if p < 0.5 and a == 0
            ),
    ...
    async def compare_models(
        ...
        with mlflow.start_run(
            ...
            mlflow.log_param("comparison_date", datetime.now(UTC).isoformat())
            ...
            if len(models_metrics) > 1:
                try:
                    ...
                    for i, (_model_id, model_data) in enumerate(models_metrics.items()):
